	...
	mentionable(o.at("mentionable").as_bool())
{
	if (auto p = o.if_contains("icon")) {
		if (auto s = p->if_string()) {
			const char *hash = p->as_string().c_str();
			icon = cUtils::Format("%srole-icons/%s/%s.%s?size=4096", DISCORD_IMAGE_BASE_URL, id.ToString(), hash,
			                      strncmp(hash, "a_", 2) == 0 ? "gif" : "png");
		}
	}
	if (auto p = o.if_contains("unicode_emoji")) {
		if (auto s = p->if_string())
			unicode_emoji = p->as_string().c_str();
	}
