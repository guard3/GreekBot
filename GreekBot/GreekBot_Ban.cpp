...
cGreekBot::process_ban(cAppCmdInteraction& i) HANDLER_BEGIN {
	...
	if (i.GetMember()->GetPermissions().TestNone(ePermission::BanMembers))
...
cGreekBot::process_unban(cInteraction& i, cSnowflake user_id) HANDLER_BEGIN {
	...
	if (i.GetMember()->GetPermissions().TestNone(ePermission::BanMembers)) [[unlikely]]
		co_return co_await InteractionSendMessage(i, MISSING_PERMISSION_MESSAGE);
	...
	switch (sc) {
		...
		case SUBCMD_GREEK: {
			static const std::string_view goodbyes[] {
				...
				"Σκέψου ότι είμαι ο Σόλωνας και αυτό π θα σ κάνω είναι σεισαχθεια της κοινωνίας απ την ομοφυλοφιλια σ"
			};
			reason = "Greek troll";
